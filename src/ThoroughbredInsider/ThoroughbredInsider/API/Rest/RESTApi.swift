...
class RESTApi {
    ...
    private static var __once: () = {
        Static.instance = RESTApi(baseUrl: Configuration.apiBaseUrl)
    }()
    ...
    internal init(baseUrl: String) {
        ...
        accessToken = RESTApi.keychain[kAccessToken]
    }

    /// the authorization header
    fileprivate let AUTH_HEADER = "Authorization"

    /// the reference to Keychain
    public static let keychain = Keychain(service: "ThoroughbredInsider")
    ...
    func setAccessToken(_ token: String?) {
        accessToken = token
        RESTApi.keychain[kAccessToken] = token
    }
    ...
    func sendRequestAndParseJson(request: URLRequest, log: Bool? = nil, success: @escaping (JSON)->(), failure: @escaping RestFailureCallback) {
        sendRequest(request: request, log: log, success: { (data, code) in
            ...
            var json: JSON!
            if data.count > 0 {
                json = JSON(data: data, options: JSONSerialization.ReadingOptions.allowFragments)
            }
            else {
            ...
            DispatchQueue.main.async {
                if code >= 400 { // check common errors in response
                    ...
                    if self.extraAllowedHttpCodes.contains(code) && self.endpointsForExtraAllowedHttpCodes.contains(endpoint) {
                        ...
                        let showMessage: ()->() = {
                            LoadingView.lastLoadingView?.terminate()
